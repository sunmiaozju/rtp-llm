...
class MasterClient:
    ...
    def __init__(self, max_connect_pool_size: int = 1000):
        self.max_connect_pool_size = max_connect_pool_size
        self._session: Optional[aiohttp.ClientSession] = None

    async def _get_session(self) -> aiohttp.ClientSession:
        if self._session is None or self._session.closed:
            timeout = ClientTimeout(total=DEFAULT_REQUEST_TIMEOUT_SEC)
    ...
    async def get_backend_role_addrs(
        ...
        debug: bool,
        generate_timeout_ms: int,
        request_priority: int = DEFAULT_REQUEST_PRIORITY,
        request_id: int = -1,
        slave_addr: Optional[str] = None,
    ) -> FlexlbResponse:
        """
        ...
        if generate_timeout_ms > 0:
            payload["generate_timeout"] = generate_timeout_ms

        resp = await self._send_schedule_request(
            master_addr, payload, generate_timeout_ms, request_id
        )
        ...
        if resp.connection_failed and slave_addr:
            ...
            resp = await self._send_schedule_request(
                slave_addr, payload, generate_timeout_ms, request_id
            )
