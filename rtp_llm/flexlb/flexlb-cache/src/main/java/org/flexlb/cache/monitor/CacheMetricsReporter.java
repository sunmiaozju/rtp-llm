...
/**
 * 缓存监控指标上报器 统一管理所有缓存相关的监控指标注册和上报逻辑
 *
 * <p>内存估算基于以下JVM参数假设：
 *
 * <ul>
 *   <li>64位JVM
 *   <li>压缩指针启用 (CompressedOops=true)
 *   <li>8字节内存对齐
 *   <li>对象头结构: Mark Word(8字节) + Class Pointer(4字节) = 12字节
 * </ul>
 *
 * <p>关键对象内存占用：
 *
 * <ul>
 *   <li>Long包装对象: 24字节 (对象头12 + long值8 + 对齐4)
 *   <li>String对象(15字符): 72字节 (String对象24 + char[]数组48)
 *   <li>ConcurrentHashMap.Node: 32字节 (对象头12 + 4个字段16 + 对齐4)
 * </ul>
 *
 * @author FlexLB
 */
...
public class CacheMetricsReporter {
  ...
  @Autowired private FlexMonitor monitor;
  ...
  /**
   * 计算String对象的内存占用 (基于字符数) String对象 + char数组的总开销
   *
  ...
  /**
   * 计算引擎缓存占用的字节数 精确估算：KeySetView<Long, Boolean> 基于64位JVM + 压缩指针的内存模型
   *
   * @param cacheCount 缓存数量
   * @return 估算的字节数
   */
  ...
  /**
   * 计算全局缓存占用的字节数 精确估算：ConcurrentHashMap<Long, Set<String>> 基于64位JVM + 压缩指针的完整内存模型
   *
   ...
   * @param success 是否成功
   */
  public void reportFindMatchingEnginesRT(
      String modelName, RoleType roleType, long startTime, String success) {
    FlexMetricTags tags =
        FlexMetricTags.of(
            "model", modelName,
            "role", roleType.name(),
  ...
  public void reportUpdateEngineBlockCacheRT(
      String engineIpPort, String role, long startTime, String success) {
    FlexMetricTags tags =
        FlexMetricTags.of(
    ...
    monitor.report(
        CACHE_UPDATE_ENGINE_BLOCK_CACHE_RT, tags, System.currentTimeMillis() - startTime);
  }
  ...
  public void reportCacheDiffMetrics(
      String engineIp, String role, int addedBlocksSize, int removedBlocksSize) {
    if (engineIp == null) {
      return;
    }

    FlexMetricTags tags =
        FlexMetricTags.of("engineIp", engineIp, "role", role != null ? role : "unknown");
