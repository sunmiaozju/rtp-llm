...
/**
 * Default implementation of DynamicIntervalManager. Provides thread-safe dynamic interval
 * adjustment based on cache diff statistics.
 *
 * @author FlexLB
 */
@Service
...
public class DefaultDynamicCacheIntervalService implements DynamicCacheIntervalService {
  ...
  public DefaultDynamicCacheIntervalService() {
    ...
    this.targetDiffSize =
        Optional.ofNullable(System.getenv("CACHE_STATUS_DIFF_SIZE"))
            .map(Integer::parseInt)
            .orElse(30);

    this.minIntervalMs =
        Optional.ofNullable(System.getenv("CACHE_STATUS_MIN_INTERVAL_MS"))
            .map(Long::parseLong)
            .orElse(50L);

    this.maxIntervalMs =
        Optional.ofNullable(System.getenv("CACHE_STATUS_MAX_INTERVAL_MS"))
            .map(Long::parseLong)
            .orElse(3000L);

    log.info(
        "DefaultDynamicIntervalManager initialized - target:{}, min:{}ms, max:{}ms",
        targetDiffSize,
        minIntervalMs,
        maxIntervalMs);
  }
