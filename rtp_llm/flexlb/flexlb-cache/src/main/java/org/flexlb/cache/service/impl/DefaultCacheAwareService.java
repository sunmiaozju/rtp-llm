...
/**
 * 缓存感知服务默认实现 提供统一的缓存管理服务，封装底层的KvCacheManager
 *
 * @author FlexLB
 */
@Slf4j
...
public class DefaultCacheAwareService implements CacheAwareService {
  ...
  @Autowired private KvCacheManager kvCacheManager;

  @Autowired private CacheMetricsReporter cacheMetricsReporter;

  @Override
  public Map<String, Integer> findMatchingEngines(
      List<Long> blockCacheKeys, String modelName, RoleType roleType, String group) {
    ...
    try {
      if (workerStatus.getCacheStatus() == null) {
        WorkerCacheUpdateResult result =
            buildFailureResult(engineIpPort, "Worker Cache Status is null");
      ...
      if (cacheStatus.getCachedKeys() == null) {
        WorkerCacheUpdateResult result =
            buildFailureResult(engineIpPort, "Worker Cached Keys is null");
        cacheMetricsReporter.reportUpdateEngineBlockCacheRT(engineIpPort, role, startTime, "0");
        return result;
      }
  ...
  private WorkerCacheUpdateResult buildSuccessResult(
      WorkerStatus workerStatus, CacheStatus cacheStatus) {
    return WorkerCacheUpdateResult.builder()
        .success(true)
        .engineIpPort(workerStatus.getIpPort())
        .cacheBlockCount(
            cacheStatus.getCachedKeys() != null ? cacheStatus.getCachedKeys().size() : 0)
