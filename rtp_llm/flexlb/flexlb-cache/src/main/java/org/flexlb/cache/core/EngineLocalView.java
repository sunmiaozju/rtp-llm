...
/**
 * 引擎本地视图 (小Hash表) 管理每个引擎的本地缓存状态和元数据 存储结构: EngineIpPort -> HashMap<Long>
 *
 * @author FlexLB
 */
@Slf4j
...
public class EngineLocalView {
  ...
  /** 动态同步间隔管理器 */
  @Autowired private DynamicCacheIntervalService dynamicIntervalManager;
  ...
  public DiffResult calculateDiff(String engineIPort, Set<Long> newCacheBlocks, String role) {
    ...
    cacheMetricsReporter.reportCacheDiffMetrics(
        engineIPort, role, addedBlocks.size(), removedBlocks.size());
  ...
  public void addOrUpdateCacheBlock(String engineIPort, Long blockCacheKey) {
    ...
    Set<Long> engineCache =
        engineViews.computeIfAbsent(engineIPort, k -> ConcurrentHashMap.newKeySet());
