...
/**
 * @author saichen description: date: 2025/3/16
 */
@Slf4j
@Component
public class HttpMasterLoadBalanceServer implements ShutdownListener, OnlineListener {
  ...
  public HttpMasterLoadBalanceServer(
      ...
      AppStateHookServer appStateHookServer,
  ...
  @Autowired private WhaleSpanUtils whaleSpanUtils;

  @Bean
  public RouterFunction<ServerResponse> loadBalancePrefill() {
    return route()
        .POST("/rtp_llm/schedule", accept(MediaType.APPLICATION_JSON), this::selectRole)
        .POST("/rtp_llm/master", accept(MediaType.APPLICATION_JSON), this::responseMasterIp)
        .POST("/rtp_llm/schedule_snapshot", accept(MediaType.APPLICATION_JSON), this::dumpLBStatus)
        .POST("/rtp_llm/notify_master", accept(MediaType.APPLICATION_JSON), this::notifyParticipant)
        .POST("/rtp_llm/update_log_level", accept(MediaType.APPLICATION_JSON), this::debugMode)
        .build();
  }

  private Mono<ServerResponse> debugMode(ServerRequest serverRequest) {
    return serverRequest
        .bodyToMono(LogLevelUpdateRequest.class)
        .flatMap(
            logLevelUpdateRequest -> {
              ...
              return ServerResponse.ok()
                  ...
                  .body(
                      Mono.just("Success! logLevel=" + WhaleMasterConfig.getLogLevel()),
                      String.class);
            })
        .onErrorResume(
            e -> {
  ...
  private Mono<ServerResponse> responseMasterIp(ServerRequest request) {
    ...
    return request
        .bodyToMono(MasterRequest.class)
        .flatMap(
            (Function<MasterRequest, Mono<ServerResponse>>)
                req -> {
  ...
  private Mono<ServerResponse> forward2Master(URI uri, MasterRequest body) {
    return generalHttpNettyService
        .request(body, uri, "/rtp_llm/schedule", MasterResponse.class)
        ...
        .onErrorResume(
            ...
            e -> {
              ...
              MasterResponse timeoutResponse =
                  MasterResponse.code(StrategyErrorType.CONNECT_TIMEOUT.getErrorCode());
        ...
        .onErrorResume(
            e -> {
              ...
              MasterResponse errorResponse =
                  MasterResponse.code(StrategyErrorType.CONNECT_FAILED.getErrorCode());
            ...
            (Function<MasterRequest, Mono<ServerResponse>>)
                req -> {
                  ...
                  if (result.isSuccess()) {
                    ...
                    balanceContext.setSuccess(false);
                    balanceContext.setPvLogData(
                        PvLogData.error(req, "error_code:" + result.getErrorCode()));
                    return ServerResponse.status(500)
                        .contentType(MediaType.APPLICATION_JSON)
  ...
  public Mono<ServerResponse> notifyParticipant(ServerRequest request) {
    return request
        .bodyToMono(MasterChangeNotifyReq.class)
        .flatMap(
            masterChangeNotifyReq -> {
              MasterChangeNotifyResp resp =
        ...
        .onErrorResume(
            (Function<Throwable, Mono<ServerResponse>>)
                e -> {
  ...
  public Mono<ServerResponse> dumpLBStatus(ServerRequest request) {
    return request
        .bodyToMono(SyncLBStatusReq.class)
        .flatMap(
            syncLBStatusReq -> {
  ...
  private RequestContext buildRequestContext(ServerRequest request) {
    ...
    for (Map.Entry<String, List<String>> entry : httpHeaders.asHttpHeaders().entrySet()) {
      ...
      BiConsumer<RequestContext, String> processor =
          HttpRequestUtils.HEADER_PROCESSORS.get(lowerCaseHeaderName);
      if (processor != null) {
