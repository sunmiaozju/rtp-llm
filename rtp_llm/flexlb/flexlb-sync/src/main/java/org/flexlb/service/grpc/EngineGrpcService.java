...
public class EngineGrpcService {
  ...
  @Getter private final EngineGrpcClient engineGrpcClient;
  ...
  public EngineRpcService.CacheStatusPB getCacheStatus(
    ...
    boolean needCacheKeys =
        RoleType.PREFILL.matches(workerStatus.getRole())
            || RoleType.PDFUSION.matches(workerStatus.getRole());
    EngineRpcService.CacheVersionPB request =
        EngineRpcService.CacheVersionPB.newBuilder()
            .setLatestCacheVersion((int) cacheVersion)
            .setNeedCacheKeys(needCacheKeys)
            .build();
    logger.info(
        "Get cache status Request: {}, cacheVersion: {}, needCacheKeys: {}",
        ip,
        cacheVersion,
        needCacheKeys);
