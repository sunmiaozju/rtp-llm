...
public class GrpcWorkerStatusRunner implements Runnable {
  ...
  public void run() {
    ...
    long latestFinishedTaskVersion =
        Optional.ofNullable(workerStatus)
  ...
  private WorkerStatusResponse launchGrpcStatusCheck(
      String ip, int grpcPort, long latestFinishedTaskVersion) {
    try {
      EngineRpcService.WorkerStatusPB workerStatusPB =
          engineGrpcService.getWorkerStatus(
              ip, grpcPort, latestFinishedTaskVersion, syncRequestTimeoutMs);
  ...
  private void handleStatusResponse(WorkerStatusResponse newWorkerStatus, long startTime) {
    try {
      ...
      engineHealthReporter.reportStatusCheckRemoteInfo(
          modelName, ipPort, newWorkerStatus.getRole(), startTime);
      ...
      if (currentVersion >= responseVersion) {
        ...
        logger.info(
            "query engine worker status via gRPC, version is not updated, currentVersion: {}, responseVersion: {}",
            currentVersion,
            responseVersion);
        return;
      }
      ...
      // 更新排队和运行中的任务
      workerStatus.updateRunningTaskList(newWorkerStatus.getRunningTaskInfo());
      // 更新完结的任务
      workerStatus.clearFinishedTask(newWorkerStatus.getFinishedTaskList());
  ...
  private void logWorkerStatusUpdate(long startTime, WorkerStatus workerStatus) {
    logger.info(
        "gRPC Worker Status - {}, role:{}, running_queue_tokens:{}, cost:{}",
        ipPort,
