...
public class ZookeeperMasterElectService implements MasterElectService, LeaderSelectorListener {
  ...
  @Setter private LBConsistencyConfig lbConsistencyConfig;
  ...
  @Getter private volatile boolean isMaster;
  ...
  public ZookeeperMasterElectService(
      GeneralHttpNettyService generalHttpNettyService, EngineHealthReporter engineHealthReporter) {
  ...
  private void initializeZookeeperClient() {
    try {
      LBConsistencyConfig.ZookeeperConfig zookeeperConfig =
          lbConsistencyConfig.getZookeeperConfig();
      client =
      ...
      LOGGER.warn(
          "Failed to initialize Zookeeper client and leader selector for roleId: {}, currentHost: {}",
          roleId,
          ip,
          e);
      closeClient();
  ...
  /**
   * 当前节点已经被选为主节点时的回调方法。 该方法需要阻塞，以保持主节点的身份，直到需要释放主节点的时候，才可以返回
   *
      ...
      try {
        ...
        if (StringUtils.isNotBlank(leaderId)) {
          if (!leaderId.equals(masterHost)) {
            LOGGER.warn(
                "ZKMasterElector roleId:{} currentHost:{} leaderId change from {} to {}.",
                roleId,
                ip,
                masterHost,
                leaderId);
  ...
  private void activelyNotifyParticipants() {
    try {
      ...
      LOGGER.warn(
          "ZKMasterElector roleId:{} currentHost:{} activelyNotifyParticipants error.",
          roleId,
          ip,
          e);
    }
  ...
  private void notifyParticipant(String participantIp) {
    try {
      LOGGER.warn(
          "ZKMasterElector roleId:{} currentHost:{} notifyParticipant:{}",
          roleId,
          ip,
          participantIp);
      ...
      Mono<MasterChangeNotifyResp> mono =
          generalHttpNettyService.request(
              req, uri, MASTER_CHANGE_NOTIFY_PATH, MasterChangeNotifyResp.class);
      mono.timeout(Duration.ofMillis(1000))
          .toFuture()
          .whenComplete(
              (masterChangeNotifyResp, throwable) ->
                  LOGGER.warn(
                      "ZKMasterElector roleId:{} currentHost:{} notifyParticipant resp:{}",
                      roleId,
                      ip,
                      masterChangeNotifyResp,
                      throwable));
    } catch (Exception e) {
      LOGGER.warn(
          "ZKMasterElector roleId:{} currentHost:{} notifyParticipant error.", roleId, ip, e);
    }
  }
  ...
  private void closeLeaderSelector() {
    if (leaderSelector != null) {
      try {
        ...
        LOGGER.warn(
            "ZKMasterElector roleId:{} currentHost:{} closeLeaderSelector error.", roleId, ip, e);
      }
    }
  }
