...
public class DefaultScheduler implements Scheduler {
  ...
  public DefaultScheduler(ConfigService configService) {
    prefillLoadBalancer =
        LoadBalanceStrategyFactory.getLoadBalanceStrategy(
            configService.loadBalanceConfig().getLoadBalanceStrategy());
    decodeLoadBalancer =
        LoadBalanceStrategyFactory.getLoadBalanceStrategy(LoadBalanceStrategyEnum.WEIGHTED_CACHE);
    vitLoadBalancer =
        LoadBalanceStrategyFactory.getLoadBalanceStrategy(
            configService.loadBalanceConfig().getLoadBalanceStrategy());
    fusionLoadBalancer =
  ...
  public MasterResponse select(BalanceContext balanceContext) {
    ...
    Map<String /*modelName*/, ModelWorkerStatus> targetModelRoleWorkerStatusMap =
        EngineWorkerStatus.MODEL_ROLE_WORKER_STATUS_MAP;
    ...
    if (roleTypeList.contains(RoleType.PREFILL)) {
      ServerStatus prefillServerStatus =
          prefillLoadBalancer.select(balanceContext, RoleType.PREFILL, null);
      if (!prefillServerStatus.isSuccess()) {
        ...
        masterResponse.setErrorCode(
            StrategyErrorType.NO_PREFILL_WORKER.getErrorMsg()
                + " : "
                + prefillServerStatus.getMessage());
      ...
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("prefill_server_ip", prefillServerStatus.getServerIp());
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("prefill_time", String.valueOf(prefillServerStatus.getPrefillTime()));
      serverStatusList.add(prefillServerStatus);
      group = prefillServerStatus.getGroup();
      ServerStatus decodeServerStatus =
          decodeLoadBalancer.select(balanceContext, RoleType.DECODE, group);
      if (!decodeServerStatus.isSuccess()) {
        ...
        masterResponse.setErrorCode(
            StrategyErrorType.NO_DECODE_WORKER.getErrorMsg()
                + " : "
                + decodeServerStatus.getMessage());
      ...
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("decode_server_ip", decodeServerStatus.getServerIp());
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("prefill_time", String.valueOf(decodeServerStatus.getPrefillTime()));
      serverStatusList.add(decodeServerStatus);

    } else if (roleTypeList.contains(RoleType.PDFUSION)) {
      ServerStatus fusionServerStatus =
          fusionLoadBalancer.select(balanceContext, RoleType.PDFUSION, null);
      ...
      if (!fusionServerStatus.isSuccess()) {
        ...
        masterResponse.setErrorCode(
            StrategyErrorType.NO_PDFUSION_WORKER.getErrorMsg()
                + " : "
                + fusionServerStatus.getMessage());
      ...
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("fusion_server_ip", fusionServerStatus.getServerIp());
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("prefill_time", String.valueOf(fusionServerStatus.getPrefillTime()));
    ...
    if (roleTypeList.contains(RoleType.VIT)) {
      ...
      if (!vitServerStatus.isSuccess()) {
        ...
        masterResponse.setErrorCode(
            StrategyErrorType.NO_VIT_WORKER.getErrorMsg() + " : " + vitServerStatus.getMessage());
      ...
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("vit_server_ip", vitServerStatus.getServerIp());
      balanceContext
          .getRequestContext()
          .getSpan()
          .setAttribute("prefill_time", String.valueOf(vitServerStatus.getPrefillTime()));
    ...
    balanceContext
        .getRequestContext()
        .getSpan()
        .addEvent("load balance finish(" + costTime + "ms)");
