...
public class GrpcCacheStatusCheckRunner implements Runnable {
  ...
  public GrpcCacheStatusCheckRunner(
      String modelName,
      String ipPort,
      String site,
      RoleType roleType,
    ...
    this.debug =
        Optional.ofNullable(System.getenv("WHALE_CACHE_DEBUG_MODE"))
  ...
  public void run() {
    ...
    if ((RoleType.PREFILL.equals(roleType) || RoleType.PDFUSION.equals(roleType))
      ...
      logger.info(
          "Skip prefill cache status check for {} because not in {}ms interval",
          ipPort,
          prefillCacheStatusCheckInterval);
      return;
    }
  ...
  private CacheStatus launchGrpcCacheStatusCheck(String ip, int grpcPort, long cacheVersion) {
    try {
      ...
      EngineRpcService.CacheStatusPB cacheStatus =
          engineGrpcService.getCacheStatus(
              ip, grpcPort, workerStatus, cacheVersion, requestTimeoutMs);
      logger.info(
          "gRPC Cache Status Response - handled for {}, role:{}, cache_key_size:{}, cache_version:{}, "
              + "available_kv_cache:{}, total_kv_cache:{}, block_size:{}",
          ipPort,
          roleType.name(),
          cacheStatus.getCacheKeysMap().size(),
  ...
  private void handleCacheStatusResponse(CacheStatus newCacheStatus, long startTime) {
    ...
    try {
      ...
      if (newCacheStatus.getMessage() != null) {
        logger.error(
            "gRPC Cache Status - {}, role:{}, message:{}",
            ipPort,
            roleType.name(),
            newCacheStatus.getMessage());
        return;
      }

      engineHealthReporter.reportCacheStatusCheckRemoteInfo(
          modelName, ipPort, roleType.name(), startTime);
      ...
      if (localTaskMapSize == 0) {
        ...
        for (Map.Entry<Long, TaskInfo> entry : localTaskMap.entrySet()) {
          cacheUsed =
              cacheUsed + entry.getValue().getInputLength() - entry.getValue().getPrefixLength();
      ...
      engineHealthReporter.reportCacheStatusCheckerFail(
          modelName, ipPort, BalanceStatusEnum.CACHE_SERVICE_UNAVAILABLE);
    }
  }

  private boolean validateCacheStatusResponse(
      WorkerStatus workerStatus, CacheStatus newCacheStatus) {
    if (debug) {
      return true;
    }
    CacheStatus currentCacheStatus = workerStatus.getCacheStatus();
    if (currentCacheStatus != null
        && newCacheStatus.getVersion() <= currentCacheStatus.getVersion()) {
      logger.info(
          "gRPC Cache Status - {}, role:{}, version not updated, current: {}, response: {}",
          ipPort,
          roleType.name(),
          currentCacheStatus.getVersion(),
  ...
  private WorkerStatus getOrCreateWorkerStatus() {
    ...
    if (workerStatus == null) {
      logger.info(
          "workerStatuses.get(ipPort) is null for cache status gRPC call, ipPort: {}", ipPort);
  ...
  private void logCacheStatusUpdate(CacheStatus cacheStatus, long startTime) {
    ...
    logger.info(
        "gRPC Cache Status - {}, role:{}, block_size:{}, version:{}, cacheKeySize:{},"
  ...
  private void updateLocalKvCache(WorkerStatus workerStatus) {
    try {
      ...
      if (!result.isSuccess()) {
        logger.warn(
            "Failed to update worker cache for IP: {}, error: {}",
            workerStatus.getIp(),
            result.getErrorMessage());
        engineHealthReporter.reportCacheStatusCheckerFail(
            modelName, ipPort, BalanceStatusEnum.CACHE_UPDATE_FAILED);
      }
    } catch (Exception e) {
      logger.warn(
          "Exception to update worker cache for IP: {}, error: {}",
          workerStatus.getIp(),
          e.getMessage());
      engineHealthReporter.reportCacheStatusCheckerFail(
          modelName, ipPort, BalanceStatusEnum.CACHE_UPDATE_FAILED);
  ...
  private void handleException(Throwable ex) {
    ...
    if (ex.getMessage() != null
        && ex.getMessage().toLowerCase().contains(DEADLINE_EXCEEDED_MESSAGE.toLowerCase())) {
      engineHealthReporter.reportCacheStatusCheckerFail(
          modelName, ipPort, BalanceStatusEnum.CACHE_GRPC_TIMEOUT);
    } else {
      engineHealthReporter.reportCacheStatusCheckerFail(
          modelName, ipPort, BalanceStatusEnum.CACHE_SERVICE_UNAVAILABLE);
    }
  ...
  private long getCurrentCacheVersion() {
    return debug
        ? -1L
        : Optional.ofNullable(workerStatuses.get(ipPort))
