...
public class WorkerAddressService {
  ...
  /** 服务发现请求线程池 */
  public static final ExecutorService serviceDiscoveryExecutor =
  ...
  public List<WorkerHost> getEngineWorkerList(String modelName, RoleType modelEndpointType) {
    ServiceRoute serviceRoute =
        modelMetaConfig.getServiceRoute(IdUtils.getServiceIdByModelName(modelName));
    ...
    List<Pair<String, Endpoint>> endpoints =
        serviceRoute.getAllEndpointsWithGroup(modelEndpointType);
    for (Pair<String, Endpoint> endpointTuple : endpoints) {
      ...
      workerHosts.addAll(
          convertServiceDiscoveryHosts(
              getServiceHosts(modelName, address), endpoint.getProtocol(), groupName));
    }
    return workerHosts;
  }
  ...
  public List<WorkerHost> getServiceHosts(String modelName, String address) {
    ...
    ServiceDiscoveryRunner serviceDiscoveryRunner =
        new ServiceDiscoveryRunner(modelName, address, engineHealthReporter, serviceDiscovery);
    ...
    try {
      ...
      if (e instanceof TimeoutException) {
        logger.error(
            "query service discovery timeout, model={}, address={}, msg:{}",
            modelName,
            address,
            "timeout");
        engineHealthReporter.reportStatusCheckerFail(
            modelName, BalanceStatusEnum.SERVICE_DISCOVERY_TIMEOUT);
      } else {
        logger.error(
            "query service discovery error, model={}, address={}, msg:{}",
            modelName,
            address,
            e.getMessage());
        engineHealthReporter.reportStatusCheckerFail(
            modelName, BalanceStatusEnum.SERVICE_DISCOVERY_ERROR);
      }
      future.cancel(true);
      return new ArrayList<>();
    }
  ...
  public static class ServiceDiscoveryRunner implements Callable<List<WorkerHost>> {
    ...
    public List<WorkerHost> call() {
      ...
      try {
        ...
        logger.error(
            "query service discovery exception, cost={}ms, model={}, address={}, msg:{}",
