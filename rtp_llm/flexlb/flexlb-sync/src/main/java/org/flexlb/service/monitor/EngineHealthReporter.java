...
public class EngineHealthReporter {
  ...
  public void init() {
    ...
    this.monitor.register(
        PREFILL_BALANCE_TOKENIZE_COST,
        FlexMetricType.GAUGE,
        FlexStatisticsType.SUMMARY | FlexStatisticsType.SUM);
    ...
    this.monitor.register(
        ENGINE_WORKER_INFO_STEP_LATENCY_VAR, FlexMetricType.GAUGE, FlexStatisticsType.SUMMARY);
    this.monitor.register(
        ENGINE_WORKER_INFO_RUNNING_QUERY_LEN_VAR, FlexMetricType.GAUGE, FlexStatisticsType.SUMMARY);
  ...
  public void reportLatencyMetric(String modelName, String role, double result, double result2) {
    ...
    logger.debug(
        "Latency metric - model: {}, role: {}, stepLatency: {}, queryLen: {}",
        modelName,
        role,
        result,
        result2);
  ...
  private void reportEngineMetric() {
    for (Map.Entry<String, ModelWorkerStatus> entry :
        EngineWorkerStatus.MODEL_ROLE_WORKER_STATUS_MAP.entrySet()) {
      ...
      monitor.report(
          ENGINE_PREFILL_WORKER_NUMBER, tags, modelWorkerStatus.getPrefillStatusMap().size());
      monitor.report(
          ENGINE_DECODE_WORKER_NUMBER, tags, modelWorkerStatus.getDecodeStatusMap().size());
    }
    ...
    reportThreadPoolInfo(
        ENGINE_BALANCING_THREAD_POOL_INFO,
        "gRpcExecutor",
        (ThreadPoolExecutor) engineGrpcClient.getExecutor());
  ...
  public void reportCacheStatusCheckRemoteInfo(
    ...
    FlexMetricTags metricTags =
        FlexMetricTags.of(
            ...
            "role", role);
    monitor.report(
        CACHE_STATUS_CHECK_VISITOR_RT, metricTags, System.currentTimeMillis() - startTime);
  ...
  public void reportStatusCheckerFail(String modelName, BalanceStatusEnum errorEnum) {
    FlexMetricTags metricTags =
        FlexMetricTags.of("model", modelName, "code", String.valueOf(errorEnum.getCode()));
    monitor.report(ENGINE_STATUS_CHECK_FAIL, metricTags, 1.0);
  }

  public void reportCacheStatusCheckerFail(
      String modelName, String engineIp, BalanceStatusEnum errorEnum) {
    FlexMetricTags metricTags =
        FlexMetricTags.of(
            "model", modelName,
            "engineIp", engineIp,
  ...
  public void reportStatusCheckerSuccess(String modelName, WorkerStatus workerStatus) {
    ...
    if (lastUpdateTime > 0) {
      monitor.report(
          ENGINE_STATUS_CHECK_SUCCESS_PERIOD,
          metricTags,
  ...
  public void reportBalancingService(BalanceContext ctx) {
    ...
    if (!ctx.isSuccess()) {
      ...
      monitor.report(ENGINE_BALANCING_MASTER_ALL_QPS, metricTags, 1.0);
      monitor.report(
          ENGINE_BALANCING_MASTER_SCHEDULE_RT,
          metricTags,
          System.currentTimeMillis() - ctx.getStartTime());
    }

    // 汇报服务器状态选择结果（根据 roleType 和 ip 区分）
    if (ctx.getMasterResponse() != null
        && CollectionUtils.isNotEmpty(ctx.getMasterResponse().getServerStatus())) {
      ...
      for (ServerStatus serverStatus : ctx.getMasterResponse().getServerStatus()) {
        if (serverStatus.getRole() != null && serverStatus.getServerIp() != null) {
          // 汇报具体服务器选择QPS
          FlexMetricTags serverSelectionTags =
              FlexMetricTags.of(
                  "model", modelName,
  ...
  public void reportPrefillBalanceSelectMetric(
      String modelName,
      boolean success,
      String errorCode,
      long totalCost,
      ...
      long calcPrefixCost,
      long calcTtftCost) {
    FlexMetricTags metricTags = FlexMetricTags.of("model", modelName);
    monitor.report(PREFILL_BALANCE_SELECT_QPS, metricTags, 1.0);
    if (!success) {
      FlexMetricTags failMetricTags =
          FlexMetricTags.of(
              "model", modelName,
              "code", errorCode);
    ...
    if (totalCost > 0) {
      monitor.report(
          PREFILL_BALANCE_TOKENIZE_COST,
          FlexMetricTags.of("model", modelName, "stage", "total"),
          totalCost);
    }
    if (tokenizeCost > 0) {
      monitor.report(
          PREFILL_BALANCE_TOKENIZE_COST,
          FlexMetricTags.of("model", modelName, "stage", "tokenize"),
          tokenizeCost);
    }
    if (calcPrefixCost > 0) {
      monitor.report(
          PREFILL_BALANCE_TOKENIZE_COST,
          FlexMetricTags.of("model", modelName, "stage", "calcPrefix"),
          calcPrefixCost);
    }
    if (calcTtftCost > 0) {
      monitor.report(
          PREFILL_BALANCE_TOKENIZE_COST,
          FlexMetricTags.of("model", modelName, "stage", "calcTtft"),
          calcTtftCost);
    }
  }
  ...
  public void reportThreadPoolInfo(
      String metricName, String name, ThreadPoolExecutor engineSyncExecutor) {
  ...
  private void reportEventLoopGroup(String eventLoopGroupName, EventLoopGroup eventLoopGroup) {
    ...
    monitor.report(
        org.flexlb.constant.MetricConstant.ENGINE_BALANCING_EVENT_LOOP_GROUP_INFO,
        FlexMetricTags.of(metricMap),
        totalActiveExecutorCount);
  ...
  public void reportCacheHitMetrics(
      String modelName, RoleType roleType, String engineIp, long hitTokens, double hitRatio) {
