...
public class ExpirationCleaner {
  ...
  public void cleanExpiredWorkers() {
    ModelWorkerStatus modelWorkerStatus =
        EngineWorkerStatus.MODEL_ROLE_WORKER_STATUS_MAP.get("engine_service");
  ...
  public static void doClean(Map<String, WorkerStatus> workerStatusMap) {
    ...
    for (Iterator<Map.Entry<String, WorkerStatus>> it = workerStatusMap.entrySet().iterator();
        it.hasNext(); ) {
      ...
      localTaskMap.forEach(
          (requestId, task) -> {
            if (isTaskTimeout(task, currentTime)) {
              localTaskMap.computeIfPresent(
                  requestId,
                  (k, existing) -> {
                    log.warn("Removing timeout task: {}", requestId);
                    return decrementQueueTime(
                        workerStatus.getRunningQueueTime(), existing, workerStatus.getRole());
                  });
            }
          });
    }
  ...
  private static TaskInfo decrementQueueTime(
      AtomicLong runningQueueTime, TaskInfo task, String role) {
