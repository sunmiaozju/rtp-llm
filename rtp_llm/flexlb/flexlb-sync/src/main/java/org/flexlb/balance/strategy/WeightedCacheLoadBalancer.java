...
/**
 * @author zjw description: 基于归一化缓存使用的加权随机负载均衡策略 通过计算所有worker缓存使用的平均值，进行归一化处理后加权随机选择 date: 2025/3/21
 */
...
public class WeightedCacheLoadBalancer implements LoadBalancer {
  ...
  public WeightedCacheLoadBalancer(
      ConfigService configService, EngineWorkerStatus engineWorkerStatus) {
  ...
  public ServerStatus select(BalanceContext balanceContext, RoleType roleType, String group) {
    ...
    List<WorkerStatus> workerStatusList =
        new ArrayList<>(workerStatusMap.values()).stream().filter(WorkerStatus::isAlive).toList();
    ...
    if (selectedWorker != null) {
      long prefixLength =
          calcPrefixMatchLength(
              selectedWorker.getCacheStatus(),
              balanceContext.getMasterRequest().getBlockCacheKeys());
      long needKvCacheLen = seqLen - prefixLength;

      // 更新其他状态
      return buildServerStatus(
          selectedWorker, seqLen, needKvCacheLen, roleType, balanceContext.getInterRequestId());
    }
