...
public class OtelConfiguration {
  ...
  SdkTracerProvider otelTracerProvider(
    ...
    List<SpanProcessor> processors =
        ListUtils.defaultIfNull(spanProcessors.getIfAvailable(), Lists.newArrayList());
    processors.addAll(
        ListUtils.defaultIfNull(spanExporters.getIfAvailable(), Lists.newArrayList()).stream()
            .map(spanProcessorProvider::toSpanProcessor)
            .toList());
  ...
  @Bean
  SpanProcessorProvider otelBatchSpanProcessorProvider(
    ...
    return new SpanProcessorProvider() {
      ...
      private void setBuilderProperties(
          OtelProcessorProperties otelProcessorProperties, BatchSpanProcessorBuilder builder) {
        if (otelProcessorProperties.getBatch().getExporterTimeout() != null) {
          builder.setExporterTimeout(
              otelProcessorProperties.getBatch().getExporterTimeout(), TimeUnit.MILLISECONDS);
        }
        ...
        if (otelProcessorProperties.getBatch().getScheduleDelay() != null) {
          builder.setScheduleDelay(
              otelProcessorProperties.getBatch().getScheduleDelay(), TimeUnit.MILLISECONDS);
        }
      }
    };
  }
  ...
  Resource otelResource(
      Environment env,
      ObjectProvider<List<Supplier<Resource>>> resourceProviders,
      OtelProperties otelProperties,
      OtelResourceProperties otelResourceProperties) {
    String applicationName =
        StringUtils.defaultString(
            otelProperties.getServiceName(), env.getProperty("spring.application.name"));
    Resource resource = defaultResource(applicationName, otelResourceProperties);
    List<Supplier<Resource>> resourceCustomizers =
        ListUtils.defaultIfNull(resourceProviders.getIfAvailable(), Lists.newArrayList());
  ...
  private Resource defaultResource(
      String applicationName, OtelResourceProperties otelResourceProperties) {
  ...
  SpanLimits otelSpanLimits(OtelProperties otelProperties) {
    return SpanLimits.getDefault().toBuilder()
        ...
        .setMaxNumberOfLinks(otelProperties.getMaxLinks())
        .build();
  }
