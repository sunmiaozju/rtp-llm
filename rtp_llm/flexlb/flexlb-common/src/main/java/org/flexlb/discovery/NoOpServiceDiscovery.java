...
public final class NoOpServiceDiscovery implements ServiceDiscovery {
  ...
  /** 环境变量前缀，用于配置服务地址对应的主机列表 格式：ip1:port1,ip2:port2 */
  ...
  public List<WorkerHost> getHosts(String address) {
    ...
    if (StringUtils.isBlank(hostsConfig)) {
      log.warn(
          "No hosts configuration found for address: {}, expected env var: {}", address, envKey);
      return Collections.emptyList();
    }
    try {
      List<WorkerHost> hosts =
          Arrays.stream(hostsConfig.split(","))
              .map(String::trim)
      ...
      log.error(
          "Failed to parse hosts configuration for address: {}, config: {}",
          address,
          hostsConfig,
          e);
