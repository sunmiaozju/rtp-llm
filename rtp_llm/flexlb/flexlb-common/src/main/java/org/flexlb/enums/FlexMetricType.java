...
public enum FlexMetricType {
  ...
  /** GAUGE类型 - 瞬时值指标 表示某个时间点的瞬时值，如CPU使用率、内存使用量等。 特点：值可以任意变化（增加或减少） */
  GAUGE("GAUGE", "瞬时值指标，表示某个时间点的即时数值"),
  /** COUNTER类型 - 累积计数器 表示累积的计数值，只能递增，如请求总数、错误总数等。 特点：值只能增加，不能减少（除非重置） */
  COUNTER("COUNTER", "累积计数器，值只能递增"),
  /** QPS类型 - 每秒查询率 表示每秒的请求处理速率，通常用于衡量系统吞吐量。 特点：基于时间窗口计算的速率值 */
