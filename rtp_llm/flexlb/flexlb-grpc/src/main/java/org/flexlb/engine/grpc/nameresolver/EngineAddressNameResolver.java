...
/**
 * @author saichen.sm date: 2025/9/19
 */
@Slf4j
@Component
public class EngineAddressNameResolver implements CustomNameResolver {

  private final Map<String /*address*/, List<String /*ip:port*/>> domainHostsMap =
  ...
  public void periodicHostUpdate() {
    log.info(
        "EngineAddressNameResolver performing periodic host update for domains: {}",
        serviceAddressList);
    fetchAllDomainsHosts();
  }
  ...
  private void fetchAllDomainsHosts() {
    for (String serverAddress : serviceAddressList) {
      ...
      try {
        ...
        log.info(
            "Fetched {} hosts for domain: {}", hosts != null ? hosts.size() : 0, serverAddress);
        updateDomainHosts(serverAddress, hosts);
      } catch (Exception e) {
        log.error(
            "Failed to fetch hosts for domain: {}, error: {}", serverAddress, e.getMessage(), e);
      }
    }
  }
  ...
  private List<String> initServiceAddressList(String modelConfigJson) {
    return Optional.ofNullable(modelConfigJson)
        ...
        .map(
            serviceRoute ->
                serviceRoute.getAllEndpoints().stream()
        ...
        .orElseThrow(
            () ->
                new IllegalArgumentException(
                    "serviceAddressList cannot be null, please config 'MODEL_SERVICE_CONFIG' environment variable, modelConfigJson="
                        + modelConfigJson));
  }
  ...
  private void updateDomainHosts(String address, List<WorkerHost> hostList) {
    ...
    log.info(
        "Address {} hosts updated, total aggregated hosts: {}", address, aggregatedHosts.size());
    // 更新全局机器列表并通知监听器
