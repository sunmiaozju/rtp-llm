...
public class EngineGrpcClient extends AbstractGrpcClient<RpcServiceGrpc.RpcServiceBlockingStub> {
  ...
  private <R> R executeGrpcCall(
    ...
    try {
      RpcServiceGrpc.RpcServiceBlockingStub rpcServiceStub =
          invoker.getRpcServiceStub().withDeadlineAfter(requestTimeoutMs, TimeUnit.MILLISECONDS);
      ...
      log.error(
          "Exception during {} gRPC call setup for {}:{}",
  ...
  public EngineRpcService.WorkerStatusPB getWorkerStatus(
      String ip, int port, EngineRpcService.StatusVersionPB request, long requestTimeoutMs) {
    return executeGrpcCall(
        ip,
        port,
        stub -> stub.getWorkerStatus(request),
        requestTimeoutMs,
  ...
  public EngineRpcService.CacheStatusPB getCacheStatus(
      String ip, int port, EngineRpcService.CacheVersionPB request, long requestTimeoutMs) {
    return executeGrpcCall(
        ip, port, stub -> stub.getCacheStatus(request), requestTimeoutMs, ServiceType.CACHE_STATUS);
  }
